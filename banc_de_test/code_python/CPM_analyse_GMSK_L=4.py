...
def create_gmsk_pulse(pulse_length, os):
    ...
    gt = Cst * gauss
    qt = cumulative_trapezoid(gt) * Ts  # intégration numérique cumulée
    return gt, qt
...
# Décode une trame GMSK par algorithme de Viterbi (MLSE) à partir du signal reçu
...
Signal_Fichier = "Test_GMSK_L4_1.lvm"
Bits_Fichier   = "BitsFichier_Test_gmsk_L=4"
...
for snr, ebn0_db in enumerate(snr_range):
    ...
    sl = 2 + L
    el = Nbits_payload - decision_delay - 2 - L - 9
    sl2 = 1 + L + 5
    el2 = len(detected_bits) - L - 3
