...
# Impulsion RC (Raised Cosine) + décomposition de Laurent
...
# Paramètres généraux de la modulation CPM avec impulsion RC
L = 3
h = 0.5
os = 8
M_ary = 2
...
# Impulsion RC : construction et interpolateur
# create_rc_pulse renvoie t sur [0, L] (pas centré comme le Gaussien GMSK),
# donc q_interp interpole directement sur [0, L], sans décalage de L/2.
gt, qt, t_g = create_rc_pulse(L, os)


# Interpolation de q(t) pour évaluation en des points arbitraires
def q_interp(x):
    return np.interp(x, t_g, qt, left=0.0, right=0.5)
...
# Génère le signal GMSK/CPM directement à partir des symboles (référence, sans décomposition)
...
# BER théorique (borne basée sur la distance minimale dmin2) en fonction de Eb/N0
def ber_theoretical(ebn0_db, dmin2=1.9):
...
if __name__ == "__main__":
    ...
    ber_theory = ber_theoretical(ebn0_range, dmin2=1.9)

    print("=" * 78)
    print(" BER Monte Carlo -- Récepteur de Laurent (C0+C1), RC L=3, h=0.5")
    print("=" * 78)
    ...
    ax.set_title("Récepteur de Laurent -- Impulsion RC", fontsize=13)
